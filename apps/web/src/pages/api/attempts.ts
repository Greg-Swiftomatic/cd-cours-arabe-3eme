...
export const GET: APIRoute = async ({ request, locals }) => {
  ...
  const attempts = await loadAttemptsForUser(locals.runtime.env, user.id, lessonFilter);
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  const attempts = await loadAttemptsForUser(locals.runtime.env, user.id, lessonSlug);
